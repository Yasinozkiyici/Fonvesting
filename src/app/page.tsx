...
export default async function Page({
  ...
  const initialScoresPreview = initialScores;
  const exploreUniverseTotal = initialScores?.total ?? null;
  ...
  return (
    <SitePageShell>
        ...
        <main className="mx-auto w-full min-w-0 max-w-[1320px] flex-1 px-3 py-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] sm:px-6 sm:py-5 sm:pb-7 lg:px-8">
          <MarketHeader initialData={marketData} exploreUniverseTotal={exploreUniverseTotal} />

          <HomePageClient
            initialScoresPreview={initialScoresPreview}
            initialScoresPartial={false}
