...
## Post-deploy smoke erişim stratejisi (auth-aware, minimal)

- **Amaç:** Deploy sonrası smoke koşuları auth engeline takılmadan çalışsın; ürün auth modeli değişmesin.
- **Yöntem:** Smoke scriptleri yalnızca `SMOKE_BYPASS_TOKEN` varsa bypass header+cookie gönderir; secret yoksa mevcut normal davranış korunur.
- **Secret lokasyonu:**
  - Vercel Project Secret: `VERCEL_AUTOMATION_BYPASS_SECRET` (kaynak)
  - CI runtime env: `SMOKE_BYPASS_TOKEN` (workflow üzerinden enjekte)
- **Kapsam:** `smoke:data`, `smoke:routes`, `smoke:ui:preview`, `verify:critical-routes`, `data:release:gate`, `verify:release-readiness`.
- **Sınıflandırma:** erişim hataları açık raporlanır: `auth_missing`, `auth_invalid`, `auth_forbidden`; auth dışı kalanlar `non-auth application failure`.
- **Güvenlik kuralları:**
  - Secret loglanmaz, stdout/stderr'e yazılmaz.
  - Smoke dışı kullanım yasak; kullanıcı trafiği için bypass yapılmaz.
  - Secret rotasyonu per release-cycle (veya en geç aylık) zorunlu.

## Kalıcı gate kuralı (operasyonel)

- Local/prodlike doğrulama release kararının ana kapısıdır; preview smoke ikincil sinyaldir.
- Preview koruma uyuşmazlığı (`PREVIEW_AUTH_BLOCKER`) release'i kilitlemez, advisory not olarak raporlanır.
- Remote tarafta karar değeri olan katman production-safe minimal doğrulamadır (`smoke:data` + `smoke:routes`).
- Auth/protection üzerinde güvenlik zayıflatılmaz; Deployment Protection devre dışı bırakılmaz.

## Kısa CI yorumlama şablonu

```text
GO: local/prodlike + production_safe_* PASS, final_decision=GO
NO_GO: prodlike_ui veya production_safe_* adımlarından biri FAIL, final_decision=NO_GO
GO+ADVISORY: preview_* advisory=1 (auth/protection), blocker adımlar PASS, final_decision=GO
```
