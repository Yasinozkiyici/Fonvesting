...
## Post-deploy smoke auth erişimi (operational hardening)

- Smoke erişimi için tek mekanizma: `SMOKE_BYPASS_TOKEN` (automation bypass secret).
- Scriptler secret varsa yalnız smoke isteklerinde bypass header/cookie ekler; secret yoksa mevcut davranış korunur.
- Erişim hata sınıfları:
  - `auth_missing`: secret enjekte edilmemiş
  - `auth_invalid`: secret var ama geçersiz
  - `auth_forbidden`: policy tarafından engellenmiş
  - `non-auth application failure`: auth dışı uygulama hatası
- Secret yönetimi:
  - Kaynak: Vercel secret (`VERCEL_AUTOMATION_BYPASS_SECRET`)
  - Aktarım: GitHub Actions env (`SMOKE_BYPASS_TOKEN`)
  - Rotasyon: düzenli (öneri: release döngüsü başına veya aylık)
  - Kullanım sınırı: yalnız smoke/doğrulama otomasyonu; genel amaçlı erişim için kullanılmaz.

## Kalıcı release karar politikası (2026-04-18)

- **Source of truth / blocking:** `pnpm exec tsc --noEmit`, `pnpm run test:unit`, local prodlike (`pnpm run smoke:ui:prodlike`) ve production-safe remote doğrulama.
- **Preview protected smoke:** operational signal olarak tutulur; `PREVIEW_AUTH_BLOCKER` (401/403 protection mismatch) artık release blocker değildir, raporda advisory olarak görünür.
- **Production-safe remote:** minimal ve anlamlı kalır (`smoke:data` + `smoke:routes`), auth/protection hatası burada blocker olarak değerlendirilir.
- **Güvenlik:** Deployment Protection açık kalır; bypass/public route eklenmez; auth modeli zayıflatılmaz.

## CI output template (operational)

### 1) GO
```text
[release-readiness] step=typecheck decision=GO
[release-readiness] step=unit decision=GO
[release-readiness] step=prodlike_ui decision=GO
[release-readiness] step=production_safe_data decision=GO
[release-readiness] step=production_safe_routes decision=GO
[release-readiness] final_decision=GO
```

### 2) NO_GO
```text
[release-readiness] step=prodlike_ui decision=NO_GO reason=verification_failed exit_code=1
[release-readiness] final_decision=NO_GO
```
```text
[release-readiness] step=production_safe_routes decision=NO_GO reason=verification_failed exit_code=1
[release-readiness] final_decision=NO_GO
```

### 3) GO + advisories (preview protection blocked)
```text
[release-readiness] step=typecheck decision=GO
[release-readiness] step=unit decision=GO
[release-readiness] step=prodlike_ui decision=GO
[release-readiness] step=production_safe_data decision=GO
[release-readiness] step=production_safe_routes decision=GO
[release-readiness] step=preview_data advisory=1 reason=preview_auth_or_protection_blocked exit_code=2
[release-readiness] step=preview_routes advisory=1 reason=preview_auth_or_protection_blocked exit_code=2
[release-readiness] step=preview_ui advisory=1 reason=preview_auth_or_protection_blocked exit_code=2
[release-readiness] final_decision=GO
```
